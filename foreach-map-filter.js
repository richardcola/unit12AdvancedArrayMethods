...
function vowelCount(str){
    ...
    str.toLowerCase().split('').forEach((letter) => {
      if (vowels.includes(letter)) {
        vowelCount[letter] = (vowelCount[letter] || 0) + 1;
      }
    });
    return vowelCount;   
}
...
function filterByValue(arr, key) {
    return arr.filter((object) => object.hasOwnProperty(key));
}
...
//this is really fascinating, unwordly like fastinathingskating        
    })[0];
}
...
function doubleOddNumbers(onlyOdd) {
    return onlyOdd
      .filter(function(val) {
        return val % 2 !== 0;
      })
